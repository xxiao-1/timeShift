...
class IccCamera():
    def __init__(self, camConfig, targetConfig, dataset, reprojectionSigma=1.0, showCorners=True,
        ...
        self.setupCalibrationTarget(
            targetConfig, showExtraction=showCorners, showReproj=showReproj, imageStepping=showOneStep)
    ...
    def findOrientationPriorCameraToImu(self, imu):
        ...
        for im in imu.imuData:
            ...
            if tk > poseSpline.t_min() and tk < poseSpline.t_max():
                ...
                omega_predicted = R_i_c * \
                    aopt.EuclideanExpression(
        ...
        for im in imu.imuData:
            tk = im.stamp.toSec()
            if tk > poseSpline.t_min() and tk < poseSpline.t_max():
                a_w.append(np.dot(poseSpline.orientation(
                    tk), np.dot(R_i_c, - im.alpha)))
        ...
        imu.GyroBiasPrior = (imu.GyroBiasPriorCount-1.0)/imu.GyroBiasPriorCount * \
            imu.GyroBiasPrior + 1.0/imu.GyroBiasPriorCount*b_gyro
    ...
    def initPoseSplineFromCamera(self, splineOrder=6, poseKnotsPerSecond=100, timeOffsetPadding=0.02):
        ...
        times = np.array(
            [obs.time().toSec()+self.timeshiftCamToImuPrior for obs in self.targetObservations])

        curve = np.matrix([pose.transformationToCurveValue(
            np.dot(obs.T_t_c().T(), T_c_b)) for obs in self.targetObservations]).T
        ...
        times = np.hstack((times[0] - (timeOffsetPadding * 2.0),
                          times, times[-1] + (timeOffsetPadding * 2.0)))
    ...
    def addDesignVariables(self, problem, noExtrinsics=True, noTimeCalibration=True, baselinedv_group_id=ic.HELPER_GROUP_ID):
        ...
        problem.addDesignVariable(
            self.cameraTimeToImuTimeDv, ic.CALIBRATION_GROUP_ID)
    ...
    def addCameraErrorTerms(self, problem, poseSplineDv, T_cN_b, blakeZissermanDf=0.0, timeOffsetPadding=0.0):
        ...
        for obs in self.targetObservations:
            ...
            frameTime = self.cameraTimeToImuTimeDv.toExpression() + obs.time().toSec() + \
            ...
            for pidx in range(0, imageCornerPoints.shape[1]):
                ...
                targetPoint = np.insert(
                    targetCornerPoints.transpose()[pidx], 3, 1)
...
class IccCameraChain():
    ...
    def initializeBaselines(self):
        ...
        for camNr in range(1, len(self.camList)):
            self.camList[camNr].T_extrinsic = self.chainConfig.getExtrinsicsLastCamToHere(
                camNr)
    ...
    def addDesignVariables(self, problem, noTimeCalibration=True, noChainExtrinsics=True):
        ...
        for camNr, cam in enumerate(self.camList):
            ...
            cam.addDesignVariables(
                problem, noExtrinsics, noTimeCalibration, baselinedv_group_id=baselinedv_group_id)
...
class IccImu(object):
    ...
    class ImuParameters(kc.ImuParameters):
        ...
        def printDetails(self, dest=sys.stdout):
            ...
            print >> dest, self.formatIndented(
                "    ", np.array(self.data["T_i_b"]))
            print >> dest, "  time offset with respect to IMU0: {0} [s]".format(
    ...
    def loadImuData(self):
        ...
        Rgyro = np.eye(3) * self.gyroUncertaintyDiscrete * \
            self.gyroUncertaintyDiscrete
        Raccel = np.eye(3) * self.accelUncertaintyDiscrete * \
            self.accelUncertaintyDiscrete

        # Now read the imu measurements.
        imu = []
        for timestamp, omega, alpha in self.dataset:
            timestamp = acv.Time(timestamp.toSec())
            imu.append(self.ImuMeasurement(
                timestamp, omega, alpha, Rgyro, Raccel))
            iProgress.sample()
    ...
    def addBiasMotionTerms(self, problem):
        ...
        gyroBiasMotionErr = asp.BSplineEuclideanMotionError(
            self.gyroBiasDv, Wgyro, 1)
    ...
    def findOrientationPrior(self, referenceImu):
        ...
        angularVelocity.initConstantSpline(
            startTime, endTime, knots, np.array([0., 0., 0.]))
        ...
        for im in referenceImu.imuData:
            ...
            if tk > angularVelocity.t_min() and tk < angularVelocity.t_max():
                ...
                omega_predicted = angularVelocityDv.toEuclideanExpression(
                    tk, 0)
                omega_measured = im.omega
        ...
        try:
            ...
            sm.logFatal(
                "Failed to obtain initial guess for the relative orientation!")
            sys.exit(-1)
        ...
        if self.estimateTimedelay and not self.isReferenceImu:
            ...
            def objectiveFunction(dt): return np.linalg.norm(
        ...
        for im in self.imuData:
            ...
            if tk > angularVelocity.t_min() and tk < angularVelocity.t_max():
                ...
                omega_predicted = C_i_b * \
                    angularVelocityDv.toEuclideanExpression(tk, 0)
                omega_measured = im.omega
...
class IccLidar(object):
    ...
    def loadLidarData(self):
        ...
        Lidar = []
        with open("/home/xxiao/dataset/calib/parkinglotRawData/poseLidarKalibr.txt", "r") as f:
            for line in f.readlines():
                line = line.strip('\n')
                line = line.split(' ')
                v = np.array([float(line[1]), float(line[2]), float(
                    line[3]), float(line[4]), float(line[5]), float(line[6])])
                # print(v)
    ...
    def initPoseSplineFromLidar(self, splineOrder=6, poseKnotsPerSecond=100, timeOffsetPadding=0.02):
        ...
        # Get the checkerboard times.
        times = np.array(
            [float(obs.stamp)+self.timeshiftLidarToImuPrior for obs in self.targetObservations])
        # ??? whether use t_c_b
...
class IccPoseLidar(object):

    def __init__(self):

        self.loadPoseLidarData()
        self.timeshiftPoseLidarToImuPrior = 0.0

    class LidarPoseMeasurement(object):
        def __init__(self, stamp, pose):
            self.pose = pose
            self.stamp = stamp

    def loadPoseLidarData(self):
        # print "Reading Lidar data ({0})".format(self.dataset.topic)
        Lidar = []
        with open("/home/xxiao/dataset/calib/parkinglotRawData/poseLidar.txt", "r") as f:
            for line in f.readlines():
                line = line.strip('\n')
                line = line.split(' ')
                v = np.array([float(line[1]), float(line[2]), float(
                ...
                # print(v)
                Lidar.append(self.LidarPoseMeasurement(line[0], v))

        self.PoseLidarData = Lidar

        if len(self.PoseLidarData) > 1:
            print "***"
            print "\r  Read %d Lidar readings over %.1f seconds                   " \
                % (len(Lidar), float(Lidar[-1].stamp) - float(Lidar[0].stamp))
        else:
    ...
    def findTimeshiftPoseLidarImuPrior(self, imu, verbose=False):
        ...
        def getQuaterion(pose):
            ...
            return Quaternion(q=np.array(pose[3:]))
            # return Quaternion(pose[3:])
        ...
        i = 0
        # print len(self.PoseLidarData)
        while i < len(self.PoseLidarData)-1:
            # print i
            t1 = float(self.PoseLidarData[i].stamp)
            t2 = float(self.PoseLidarData[i+1].stamp)
            tl = (t1+t2)/2
            dt = t2-t1
            # print "t_lidar"
            # print t_l
            ...
            # get lidar
            q1 = getQuaterion(self.PoseLidarData[i].pose)
            q2 = getQuaterion(self.PoseLidarData[i+1].pose)
            ...
            omega_lidar = 0.5 / dt * angle_axis[3] * angle_axis[0:3]
            # print omega_lidar

            # get imu
            j = 0
            while j < len(imu.myImuData):
                ti = float(imu.myImuData[j].stamp)
                if ti > tl:
                    break
                j = j+1

            b = 0
            if j > 0 and j < len(imu.myImuData)-1:
                b = j-1
            elif j>= len(imu.myImuData)-1:
                # print "flag"
                # print(j)
                break
            ...
            omega_imu_left = imu.myImuData[b].omega
            omega_imu_right = imu.myImuData[b+1].omega

            scale = (tl-t_imu_left)/(t_imu_right-t_imu_left)
            ...
            omega_lidar_norm = np.hstack(
                (omega_lidar_norm, np.linalg.norm(omega_lidar)))
            i+=1

        print(len(omega_imu_norm))
        print(len(omega_lidar_norm))
        print(len(t))
        ...
        Isplot = True
        if Isplot:
            pl.plot(t, omega_imu_norm, label="imu")
            # pl.plot(t, omega_lidar_norm, label="lidar")
            # pl.plot(t-shift, omega_imu_norm, label="imu_new")
            pl.legend()
            pl.title("Time shift prior lidar-imu estimation")
            pl.figure()
            pl.plot(corr)
            pl.title("Cross-correlation ||omega_predicted||, ||omega_measured||")
            pl.show()
        ...
        # store the timeshift (t_imu = t_lidar + timeshiftLidarToImuPrior)
        self.timeshiftPoseLidarToImuPrior = shift

        print "  Time shift Lidar to imu (t_imu = t_lidar + shift):"
        print self.timeshiftPoseLidarToImuPrior


class IccWheel(object):

    def __init__(self):

        self.loadWheelData()
        self.targetObservations = []
        self.timeshiftWheelToImuPrior = 0.0

    class WheelMeasurement(object):
        def __init__(self, stamp, vec):
            self.vec = vec
            self.stamp = stamp

    def loadWheelData(self):
        # print "Reading Lidar data ({0})".format(self.dataset.topic)
        Wheel = []
        with open("/home/xxiao/dataset/calib/parkinglotRawData/angvWheel.txt", "r") as f:
            for line in f.readlines():
                line = line.strip('\n')
                line = line.split(' ')
        ...
        self.WheelData = Wheel

        if len(self.WheelData) > 1:
            print "\r  Read %d Wheel readings over %.1f seconds                   " \
                % (len(Wheel), float(Wheel[-1].stamp) - float(Wheel[0].stamp))
        else:
            sm.logFatal(
                "Could not find any Wheel messages. Please check the dataset.")
            sys.exit(-1)
    ...
    #          and imu, the maximum corresponds to the timeshift...
    #          in a next step we can use the time shift to estimate the rotation between camera and imu
    def findTimeshiftWheelImuPrior(self, imu, verbose=True):
        print "Estimating time shift wheel to imu:"
        ...
        print(len(omega_measured_norm))
        if len(omega_wheel_norm) == 0 or len(omega_measured_norm) == 0:
            sm.logFatal("The time ranges of the Lidar and IMU do not overlap. "
                        "Please make sure that your sensors are synchronized correctly.")
            sys.exit(-1)

        # get the time shift
        corr = np.correlate(omega_wheel_norm, omega_measured_norm, "full")
        discrete_shift = corr.argmax() - (np.size(omega_measured_norm) - 1)
        ...
        if Isplot:
            pl.plot(t, omega_measured_norm, label="measured_raw")
            pl.plot(t, omega_wheel_norm, label="wheel")
            pl.plot(t-shift, omega_measured_norm, label="measured_corrected")
            pl.legend()
            pl.title("Time shift prior camera-imu estimation")
            pl.figure()
    ...
    def findWheelImu(self, imu):
        ...
        # Add spline representing rotational velocity of in body frame
        startTime = float(imu.myImuData[0].stamp)
        endTime = float(imu.myImuData[-1].stamp)
        knotsPerSecond = 50
        knots = int(round((endTime - startTime) * knotsPerSecond))
        ...
        # referenceAbsoluteOmega = lambda dt = np.array([0.]): \
        #         np.asarray([np.linalg.norm(angularVelocityDv.toEuclidean(float(wheel.stamp) + dt[0], 0))\
        #                     for wheel in self.WheelData \
        #                     if (float(wheel.stamp) > angularVelocity.t_min() \
        #                         and float(wheel.stamp) + dt[0] < angularVelocity.t_max())])
        dt = np.array([0])

        def referenceAbsoluteOmega():
            for wheel in self.WheelData:
                if (float(wheel.stamp) > angularVelocity.t_min() and float(wheel.stamp) + dt[0] < angularVelocity.t_max()):
                    print wheel.stamp
                    return np.asarray([np.linalg.norm(angularVelocityDv.toEuclidean(float(wheel.stamp)+dt[0], 0))])

        absoluteOmega = lambda dt = np.array([0.]): \
            np.asarray([np.linalg.norm(im.omega) for im in imu.myImuData
                        if (float(im.stamp) + dt[0] > angularVelocity.t_min()
                        and float(im.stamp) + dt[0] < angularVelocity.t_max())])
        ...
        if False:
            ...
            refined_shift = scipy.optimize.fmin(
                objectiveFunction, np.array([shift]), maxiter=100)[0]
            wheelImutimeOffset = float(refined_shift)

        print shift, "[s]"
        print "after optimize ********* "
        print wheelImutimeOffset, "[s]"

    def findWheelImu2(self, imu):
        print
        print "Estimating wheel-imu timeShift initial guess."
        ...
        for wheel in self.WheelData:
            ...
            omega_wheel = wheel.vec
            # find the imu
            i = 0
            while i < len(imu.myImuData):
                ti = float(imu.myImuData[i].stamp)
                if ti > tw:
                    break
                i = i+1
            ...
            scale = (tw-t_imu_left)/(t_imu_right-t_imu_left)
            omega_imu = scale*omega_imu_left+(1-scale)*omega_imu_right
...
class IccMyImu(object):

    def __init__(self):

        self.loadMyImuData()

    class MyIMUMeasurement(object):
        def __init__(self, stamp, omega):
            self.omega = omega
            self.stamp = stamp

    def loadMyImuData(self):
        # print "Reading Lidar data ({0})".format(self.dataset.topic)
        myIMU = []
        with open("/home/xxiao/dataset/calib/parkinglotRawData/angvImu.txt", "r") as f:
            for line in f.readlines():
                line = line.strip('\n')
                line = line.split(' ')
                v = np.array([float(line[1]), float(line[2]), float(line[3])])
                # print(v)
                myIMU.append(self.MyIMUMeasurement(line[0], v))

        self.myImuData = myIMU

        if len(self.myImuData) > 1:
            print "\r  Read %d my IMU readings over %.1f seconds                   " \
                % (len(myIMU), float(myIMU[-1].stamp) - float(myIMU[0].stamp))
        else:
            sm.logFatal(
                "Could not find any my IMU messages. Please check the dataset.")
            sys.exit(-1)
...
class IccScaledMisalignedImu(IccImu):
    ...
    class ImuParameters(IccImu.ImuParameters):
        ...
        def printDetails(self, dest=sys.stdout):
            ...
            print >> dest, "    M:"
            print >> dest, self.formatIndented(
                "      ", np.array(self.data["gyroscopes"]["M"]))
            print >> dest, "    A [(rad/s)/(m/s^2)]:"
            print >> dest, self.formatIndented(
                "      ", np.array(self.data["gyroscopes"]["A"]))
            print >> dest, "    C_gyro_i:"
            print >> dest, self.formatIndented(
    ...
    def addGyroscopeErrorTerms(self, problem, poseSplineDv, mSigma=0.0, gyroNoiseScale=1.0, g_w=None):
        ...
        for im in self.imuData:
            ...
            if tk > poseSplineDv.spline().t_min() and tk < poseSplineDv.spline().t_max():
                ...
                a_w = poseSplineDv.linearAcceleration(tk)
                r_b = self.r_b_Dv.toExpression()
                a_b = C_b_w * (a_w - g_w) + w_dot_b.cross(r_b) + \
    ...
    def addAccelerometerErrorTerms(self, problem, poseSplineDv, g_w, mSigma=0.0,
        ...
        for im in self.imuData:
            ...
            if tk > poseSplineDv.spline().t_min() and tk < poseSplineDv.spline().t_max():
                ...
                w_dot_b = poseSplineDv.angularAccelerationBodyFrame(tk)
                C_i_b = self.q_i_b_Dv.toExpression()
                rx_b = self.r_b_Dv.toExpression() + C_i_b.inverse() * \
                    self.rx_i_Dv.toExpression()
                ry_b = self.r_b_Dv.toExpression() + C_i_b.inverse() * \
                    self.ry_i_Dv.toExpression()
                rz_b = self.r_b_Dv.toExpression() + C_i_b.inverse() * \
                    self.rz_i_Dv.toExpression()
